...
class Xtq2Spider(RedisSpider):
    name = 'xtq2'
    # 不需要allowed_domains
    allowed_domains = ['xtq.zynews.cn']
    # 不需要start_urls
    # start_urls = ['http://xtq.zynews.cn/']

    redis_key = 'xtq2:start_urls'

    def __init__(self, *args, **kwargs):
        # Dynamically define the allowed domains list.
        domain = kwargs.pop('domain', '')
        self.allowed_domains = filter(None, domain.split(','))
        super(Xtq2Spider, self).__init__(*args, **kwargs)
        self.page = 1
        self.max_pages = 3


    def get_url(self):
        return 'https://xtq.zynews.cn/wzpostlist.php?mod=list'.format(self.page)
