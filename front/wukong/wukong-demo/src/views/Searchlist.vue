<template>
	...
	<div class="carslist">
		<div class="carslist-top">
			<van-nav-bar title="搜索结果列表" left-arrow @click-left="onClickLeft" @click-right="onClickRight">
			...
			<br>
			<!-- 根据分类的到的搜索结果 -->
			<div v-if="$route.params.class=='categories'" class="carslit-msg" v-for="(items,index) in datas" style="width: 100%;background-color: white;">

			<!-- 根据汽车品牌的到的搜索结果 -->
			<div @click="gotocar(items.id,item.id,days)" class="choose-car" v-for="(item,index1) in cars " v-if="item.category ==searchresult[0].id " style="background-color: white;">
			...
			<!-- 根据汽车品牌的到的搜索结果 -->
			<div v-if="$route.params.class=='cars'" class="carslit-msg" v-for="(items,index) in datas" style="width: 100%;background-color: white;">

			<div @click="gotocar(items.id,item.id,days)" class="choose-car" v-for="(item,index1) in cars " v-if="item.name ==searchresult[0].name " style="background-color: white;">
				<div v-if="item.shop == items.id" class="car">
					<van-row type="flex">
						<van-col span="8">
							<img style="width: 100%;" v-if="item.imgs" :src="item.imgs[0].img" alt="">
