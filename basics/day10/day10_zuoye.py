...
class Rectangle(Point):
    ...
    def judge(self, r_x, r_y):
        j1 = abs(r_x - self.x)
        j2 = abs(r_y - self.y)
        j3 = abs(self.width + self.y - r_y)
        j4 = abs(self.length + self.x - r_x)
        if min(j1, j2, j3, j4) == 0:
            print("在矩形上")
        elif max(j1, j4) < self.length and max( j2, j3) < self.width:
            print("在矩形内")
        else:
            print("在矩形外")
...
#
# class Count:
#     def figure(self, shape: Shape):
#         print(f"切换图形到{shape.name}")
#         shape.perimeter()
#
#
# s1 = Count()
# s1.figure(Rectangle("正方形", 2, 4))


# 5、某公司雇员（Employee）包括经理（Manager），技术人员（Technician）和销售员（Salesman）。
# 以Employee类为基类派生出Manager，Technician和Salesman类；Employee类的属性包括姓名、职工号、工资级别，月薪（实发基本工资加业绩工资）。
# 操作包括月薪计算方法（pay()），该方法要求输入请假天数，扣去应扣工资后，得出实发基本工资。
# Technician类派生的属性有每小时附加酬金和当月工作时数，及研究完成进度系数。
# 业绩工资为三者之积。也包括同名的pay()方法，工资总额为基本工资加业绩工资。
...
class Employee:
    ...
    def __init__(self, name, number, salary_scale) -> None:
        super().__init__()
        self.name = name
        self.number = number
    ...
    def pay(self, days):
        self.monthly_salary = self.salary_scale * 2000 - days * 200
        # return self.monthly_salary
        print(f"{self.name} 本月的总工资为{self.monthly_salary}")


class Technician(Employee):

    def __init__(self, name, number, salary_scale, reward, working_hours, plan_coefficient) -> None:
        super().__init__(name, number, salary_scale)
        self.reward = reward
        self.working_hours = working_hours
        self.plan_coefficient = plan_coefficient

    def pay(self, days):
        merit_pay = self.reward * self.working_hours * self.plan_coefficient
        self.monthly_salary = merit_pay + self.salary_scale * 2000 - days * 200
        # return self.monthly_salary
        print(f"{self.name} 本月的总工资为{self.monthly_salary}")
...
class Salesman(Employee):
    ...
    def __init__(self, name, number, salary_scale, sales, percent) -> None:
        super().__init__(name, number, salary_scale)
        self.sales = sales
        self.percent = percent

    def pay(self, days):
        merit_pay = self.sales * self.percent
        self.monthly_salary = merit_pay + self.salary_scale * 2000 - days * 200
        # return self.monthly_salary
        print(f"{self.name} 本月的总工资为{self.monthly_salary}")
...
class Manager(Employee):
    ...
    def __init__(self, name, number, salary_scale, bonus, performance_coefficient) -> None:
        super().__init__(name, number, salary_scale)
        self.bonus = bonus
        self.performance_coefficient = performance_coefficient

    def pay(self, days):
        merit_pay = self.bonus * self.performance_coefficient
        self.monthly_salary = merit_pay + self.salary_scale * 2000 - days * 200
        print(f"{self.name} 本月的总工资为{self.monthly_salary}")
        # return self.monthly_salary


e1 = Employee("小王", "234", 1)
e1.pay(4)
m1 = Manager("张经理", "005", 3, 5000, 0.3)
m1.pay(1)
